...
const projectPageInvertToggle = document.getElementById("invertToggle");
...
function setProjectInvertedMode(isInverted) {
  document.documentElement.classList.toggle("inverted-mode", isInverted);

  if (projectPageInvertToggle) {
    projectPageInvertToggle.setAttribute("aria-pressed", String(isInverted));
    projectPageInvertToggle.setAttribute(
      "aria-label",
      isInverted ? "turn off inverted dark mode" : "turn on inverted dark mode"
    );
  }

  try {
    window.localStorage.setItem("inverted-mode", isInverted ? "on" : "off");
  } catch (_) {}
}

function toggleProjectInvertedMode() {
  setProjectInvertedMode(!document.documentElement.classList.contains("inverted-mode"));
}

if (projectPageInvertToggle) {
  try {
    setProjectInvertedMode(window.localStorage.getItem("inverted-mode") === "on");
  } catch (_) {
    setProjectInvertedMode(document.documentElement.classList.contains("inverted-mode"));
  }

  projectPageInvertToggle.addEventListener("click", toggleProjectInvertedMode);
}
